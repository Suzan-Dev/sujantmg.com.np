---
title: The Complete Guide to Refs and useRef() in React
date: '2021-08-21'
tags: ['React', 'Hooks']
draft: false
summary: In this post, you’ll learn about accessing DOM elements & specially useRef() hook of React to create persisted mutable values.
images: ['/static/images/blogs/3.the-complete-guide-to-refs-and-useRefs-in-react/image1.jpg']
layout: PostLayout
authors: ['default']
---

![react-refs-and-useRef-hook](/static/images/blogs/3.the-complete-guide-to-refs-and-useRefs-in-react/image1.jpg)
...
`useRef(initialValue)` is a built-in React hook that accepts one argument as the initial value and returns a _reference_ (aka _ref_).
...
[Try the demo](https://codesandbox.io/s/logging-button-clicks-reference-forked-3k52h?file=/src/App.js)

`const countRef = useRef(0)` creates a references `countRef` initialized with _0_.

When the button is clicked, `handle` function is invoked and the reference value is incremented: `countRef.current++`.
The reference value is logged to the console.
...
is logged to the console just once, at initial rendering, and no re-rendering happens when the reference is updated.
...
[Try the demo](https://codesandbox.io/s/logging-button-clicks-state-forked-gusht?file=/src/App.js)

Open the demo and click the button. Each time you click,
you will see in the console the message `'I rendered!'` — meaning that each time the state is updated, the component re-renders.
...
[Try the demo](https://codesandbox.io/s/stopwatch-forked-y364g?file=/src/App.js)

`startHandler()` function, which is invoked when the Start button is clicked,
starts the timer and saves the timer id in the reference `timerIdRef.current = setInterval(...)`.

To stop the stopwatch user clicks Stop button.
The Stop button handler `stopHandler()` accesses the timer id from the reference and stops the timer `clearInterval(timerIdRef.current)`.
...
2. Assign the reference to _ref_ attribute of the element: `<div ref={elementRef}></div>;`
...
[Try the demo](https://codesandbox.io/s/access-dom-element-forked-0z2m2?file=/src/App.js)
...
To make it work you’ll need to create a reference to the input, assign the reference to _ref_ attribute of the tag,
and after mounting call the special method `element.focus()` on the element.
...
[Try the demo](https://codesandbox.io/s/input-focus-forked-9j6k0?file=/src/App.js)

`const inputRef = useRef()` creates a reference to hold the input element.

`inputRef` is then assigned to _ref_ attribute of the input field: `<input ref={inputRef} type="text" />`.
...
[Try the demo](https://codesandbox.io/s/empty-on-initial-rendering-forked-n7uc9?file=/src/App.js)
...
That’s why `inputRef.current` evaluates to `undefined` during initial rendering.

`useEffect(callback, [])` hook invokes the callback right after mounting, when the input element has already been created in DOM.

_callback_ function of the `useEffect(callback, [])` is the right place to access `inputRef.current` because it is guaranteed that the DOM is constructed.
...
Calling const `reference = useRef(initialValue)` with the initial value returns a special object named reference.
The reference object has a property _current_: you can use this property to read the reference value `reference.current`, or update `reference.current = newValue`.
...
`<div ref={reference}>Element</div>` — and the element is available at `reference.current`.

> **Challenge**: write a custom hook `useEffectSkipFirstRender()` that works as `useEffect()`,
> only that it doesn’t invoke the callback after initial rendering (Hint: you need to use `useRef()`). Share your solution in a comment below!
